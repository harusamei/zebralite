...
class ScmaLoader:
    ...
    # tbs_prompt的抽象，格式一样
    def get_grp_prompt(self, tb_names=None) -> str:
        ...
        # TODO 更好方法是用LLM 精简
        tStr = '\n'.join(prompts)
        return tStr[:max_prompt_len] if len(tStr) > max_prompt_len else tStr

    def gen_tbs_prompt(self, tb_names=None) -> str:
        # 所有表
        if tb_names is None or len(tb_names) == 0:
            tb_names = self.get_table_nameList()
        ...
        for table in table_list:
            ...
            if lit_flag or table['column_count'] > max_col_count:
                prompts.append(f"Columns: [{table['cols_info']}]")
            else:
                prompts.append(f"Description: {table['tb_desc']}")
                md_data = self.gen_cols_prompt(table['table_name'])
    ...
    def gen_cols_prompt(self, tb_name):

        max_col_count = 20
        col_names = self.get_column_nameList(tb_name)
        if len(col_names) > max_col_count:
            need_cols = ['column_name','column_type','val_lang']
        else:
            need_cols = ['column_name','column_type','val_lang','examples']
        col_info = []
        for col_name in col_names:
            ...
            col_info.append(t_dict)
        t_df = pd.DataFrame(col_info)
        return t_df.to_markdown(index=False, tablefmt="pipe")
...
if __name__ == '__main__':
    ...
    print(loader.gen_cols_prompt('olist_geolocation_dataset'))

    print(loader.get_tables_with_column('geolocation_lat')) 
    print('_________________________')

    with open('tem.out', 'w',encoding='utf-8') as f:
        f.write('db_info')
        f.write(loader.get_db_info())
        f.write('tbs_prompt\n')
        f.write(loader.gen_tbs_prompt())
        f.write('grp_prompt\n')
        f.write(loader.get_grp_prompt())
