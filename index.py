...
def render_sidebar():
    ...
    cont_body = st.container(height=400,border=True)
...
def show_talk(cont=None):
    ...
    with cont:
        for mes in st.session_state.messages[-5:]:
            if 'request' in mes['type']:
                st.write(f"user: {mes.get('msg')}")
            else:
                st.write(f"agent: {mes.get('reply')}")
...
async def apply():
    ...
    answ = controller.genAnswer(pipeline)
    
    return answ

def get_db_summary():
    controller = st.session_state['controller']
    db_info = controller.parser.scha_loader.get_db_summary()
    return db_info
